...
/**
 * @callback Cam~GetMetadataConfigurationCallback
 * @property {?Error} error
 * @property {Cam~VideoSource|Array.<Cam~VideoSource>} videoSources
 * @property {string} xml Raw SOAP response
 */
...
/**
 * Receive video sources
 * @param {Cam~GetMetadataConfigurationCallback} [callback]
 */
Cam.prototype.getMetadataConfigurations = function(callback) {
    this._request({
        service: 'media'
        , body: this._envelopeHeader() +
        '<GetMetadataConfigurations xmlns="http://www.onvif.org/ver10/media/wsdl"/>' +
        this._envelopeFooter()
    }, function(err, data, xml) {
        if (!err) {
            /**
             * @name Cam#metadataConfiguration
             */
            this.metadataConfigurations = linerase(data).getMetadataConfigurationsResponse.metadataConfigurations;
        }else {
            console.log(err);
        }
        if (callback) {
