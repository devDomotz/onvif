			...
			if (err || !data[0].probeMatches) {
				...
				if (!cams[camAddr]) {
					...
					if (options.resolve !== false) {
						var camUri = url.parse(data.probeMatches.probeMatch.XAddrs.split(' ')[0]);
						var filterEmpty = function(n) { return n; };
						var types = (data.probeMatches.probeMatch.types || '').split(' ').filter(filterEmpty);
						var scopes = (data.probeMatches.probeMatch.scopes || '').split(' ').filter(filterEmpty);
						cam = new Cam({
